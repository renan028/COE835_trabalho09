...
\end{equation}
%
onde $F^T(u,y) = \mat{u & -\omega_r^T} \in \mathbb{R}^{1 \times 4}$, ${\Psi}^T = \mat{k & \psi_1 & \psi_2 & \psi_3}$, $\omega_1 = u/\Lambda(s)$ e $\omega_2 = y/\Lambda(s)$. O polinômio $\Lambda(s)$ é encontrado através de $N_m A_0$, onde $N_m = k_m$ é o numerador de $M(s)$ e $A_0(s)$ é escolhido, e consiste no polinômio do observador. 
...
\end{equation}
%
que corresponde à mesma dinâmica da equação \eqref{eq:dotchi}. Porém, esse estimador não pode ser implementado porque não temos conhecimento sobre $\Psi$. Então, define-se um erro de estima:
%
\begin{align}
